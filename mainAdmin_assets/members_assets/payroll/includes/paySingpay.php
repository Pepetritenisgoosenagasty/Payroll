...
if(isset($_REQUEST['__l_p@i$d_']) && isset($_REQUEST['_e_i@$d'])){
    ...
    $query = mysqli_query($vSky,"SELECT emp.`emp_id`, emp.`emp_name`, emp.`emp_bank_account_no`, emp.`emp_bank_branch`,                                 emp.`emp_ssnit`, pr.*
                        FROM `payroll_store` AS pr
                        INNER JOIN `employees` AS emp ON pr.`emp_id`=emp.`emp_id` 
                        WHERE (emp.`sacked`='0' OR emp.`sacked`!='0') AND pr.`id`='$valPayID2' AND pr.`emp_id`='$valEmpID2'");
