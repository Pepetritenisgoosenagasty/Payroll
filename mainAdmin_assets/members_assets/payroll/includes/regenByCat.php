...
if(isset($_REQUEST['dDFp_d']) && isset($_REQUEST['fdp_fdp'])){
    //get invoices data
        $payroll_date = $_REQUEST['dDFp_d'];
        $cat_id = $_REQUEST['fdp_fdp'];
    
        $payroll_date_d = base64_decode($payroll_date);
        $cat_id_id = base64_decode($cat_id);
    
        $query = mysqli_query($vSky,"SELECT emp.`emp_id`, emp.`emp_name`, emp.`emp_bank_account_no`, emp.`emp_bank_branch`,                                 emp.`emp_ssnit`, py.*, mc.* FROM `payroll_store` AS py 
                                INNER JOIN `employees` AS emp ON emp.`emp_id`=py.`emp_id`
                                INNER JOIN `main_category` AS mc ON mc.`cat_id`=emp.`emp_main_cat_id`
                                WHERE (emp.`sacked`='0' OR emp.`sacked`!='0') AND emp.`emp_main_cat_id`='$cat_id_id' AND py.`date_stored`='$payroll_date_d'");
...
$pdf->Output();
        ...
        echo "<h3> <span style='text-align:center; margin-left:450px; padding-top:10px; color:red;'> Please Go Back To View Payroll To Get All Generated Payslips</span><h3>";
    }
