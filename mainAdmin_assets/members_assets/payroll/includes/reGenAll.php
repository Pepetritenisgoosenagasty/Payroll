...
if(isset($_REQUEST['duration_select2'])){
    //get invoices data
        $valPayId = $_REQUEST['duration_select2'];
    
        //$valPayID2 = base64_decode($valPayId);
    
    $query10 = mysqli_query($vSky, "SELECT * FROM `payroll_store` WHERE `date_stored`='$valPayId'");
    $countdem = mysqli_num_rows($query10);
    
    if($countdem > 0){
           
    $query = mysqli_query($vSky,"SELECT emp.`emp_id`, emp.`emp_name`, emp.`emp_bank_account_no`, emp.`emp_bank_branch`, pr.*
                        FROM `payroll_store` AS pr
                        INNER JOIN `employees` AS emp ON pr.`emp_id`= emp.`emp_id` 
                        WHERE `date_stored`='$valPayId'");
...
$c = 1;
        $query_l = mysqli_query($vSky,"SELECT emp.`emp_id`, emp.`emp_name`, emp.`emp_bank_account_no`, emp.`emp_bank_branch`, pr.*
                        FROM `payroll_store` AS pr
                        INNER JOIN `employees` AS emp ON pr.`emp_id`= emp.`emp_id` 
                        WHERE `date_stored`='$valPayId'");
