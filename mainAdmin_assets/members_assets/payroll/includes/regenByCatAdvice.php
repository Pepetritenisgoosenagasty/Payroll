...
if(isset($_REQUEST['dDFp_d']) && isset($_REQUEST['fdp_fdp'])){
    //get invoices data
        $payrollDate = $_REQUEST['dDFp_d'];
        $catId = $_REQUEST['fdp_fdp'];
    
        $valPayD2 = base64_decode($payrollDate);
        $valCatD2 = base64_decode($catId);
    
    $query10 = mysqli_query($vSky, "SELECT * FROM `payroll_store` WHERE `date_stored`='$valPayD2'");
...
$c = 1;
        $query_l = mysqli_query($vSky,"SELECT emp.`emp_id`, emp.`emp_name`, emp.`emp_bank_account_no`, emp.`emp_bank_branch`,                                 emp.`emp_ssnit`, py.*, mc.* FROM `payroll_store` AS py 
                                INNER JOIN `employees` AS emp ON emp.`emp_id`=py.`emp_id`
                                INNER JOIN `main_category` AS mc ON mc.`cat_id` = emp.`emp_main_cat_id`
                                WHERE (emp.`sacked`='0' OR emp.`sacked`!='0') AND emp.`emp_main_cat_id`='$valCatD2' AND py.`date_stored`='$valPayD2'");
...
$pdf->Output();
        ...
        echo "<h3> <span style='text-align:center; margin-left:450px; padding-top:10px; color:red;'> Please Go Back To Payroll To Report To Get All Generated Payslipss</span><h3>";
    }
