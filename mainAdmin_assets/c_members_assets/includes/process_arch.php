...
if(isset($_POST['process'])){
    ...
    if($process == "get_shortage_value"){
        ...
        $chk_for_month = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$month_no'");
        $c_dem = rows($chk_for_month);
        
        if( $c_dem > 0 ){
            /////////////////////////////// get total shortage value ////////////////////////////
        $get_values = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$month_no'");
            ...
            $year = $exploded[0];
        ///////////////////////////// get month name ////////////////////////////////////    
        $get_month_name = queryn($vSky, "SELECT DISTINCT MONTHNAME(`discharging_date`) AS month_name, YEAR(`discharging_date`) AS year FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1");
            ...
            $get_driver_no = queryn($vSky, "SELECT `driver_name`, `truck_no`, MAX(`shortage_val`) AS `max` FROM `arch_trips` WHERE (MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year') AND `shortage_val`=(SELECT MAX(`shortage_val`) FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1)");
...
if(isset($_POST['process_variable'])){
    ...
    if($process_variable == "get_variable"){
        ...
        $chk_for_month = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$shortVariable'");
        $c_dem = rows($chk_for_month);
        
        if( $c_dem > 0 ){
               $get_values = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$shortVariable'");
        ...
        $get_month_name = queryn($vSky, "SELECT DISTINCT MONTHNAME(`discharging_date`) AS month_name, YEAR(`discharging_date`) AS year FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1");
            ...
            $get_driver_no = queryn($vSky, "SELECT `driver_name`, `truck_no`, MAX(`shortage`) AS `max` FROM `arch_trips` WHERE (MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year') AND `shortage`=(SELECT MAX(`shortage`) FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1)");
...
if(isset($_POST['process_variable_overnyt'])){
    ...
    if($process_variable_overnyt == "get_variable_overnyt"){
        ...
        $chk_for_month = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$selOverNyt'");
        $c_dem = rows($chk_for_month);
        
        if( $c_dem > 0 ){
            /////////////////////////////// get total shortage value ////////////////////////////
        $get_values = queryn($vSky, "SELECT * FROM `arch_trips` WHERE DATE_FORMAT(`discharging_date`,'%Y-%m')='$selOverNyt'");
        ...
        $get_month_name = queryn($vSky, "SELECT DISTINCT MONTHNAME(`discharging_date`) AS month_name, YEAR(`discharging_date`) AS year FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1");
            ...
            $get_driver_no = queryn($vSky, "SELECT `driver_name`, `truck_no`, MAX(`overnight_allowance`) AS `max` FROM `arch_trips` WHERE (MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year') AND `overnight_allowance`=(SELECT MAX(`overnight_allowance`) FROM `arch_trips` WHERE MONTH(`discharging_date`)='$month_no' AND YEAR(`discharging_date`)='$year' LIMIT 1)");
...
if(isset($_POST['historyProcess'])){
    ...
    if($historyProcess == "get_trip_history"){
        ...
        $query_trips = queryn($vSky, "SELECT * FROM `arch_trips` WHERE `id`='$hisID' LIMIT 1");
...
if(isset($_POST['archHistoryProcess'])){
    ...
    if($archHistoryProcess == "archHistoryProcess"){
        ...
        $query_trips = queryn($vSky, "SELECT * FROM `arch_trips` WHERE `arch_id`='$archHistoryProcessID' LIMIT 1");
        ...
        while($tripsInfo = assoc($query_trips)){
         //   $sub_name = $tripsInfo['sub_name'];
          //  $main_company_id = $tripsInfo['main_company_id'];
            ...
            $t_main_name = $tripsInfo['t_name'];
            $t_address = $tripsInfo['t_adrress'];
            $t_tel = $tripsInfo['t_tel'];
            $name = $tripsInfo['name'];
            $presented_to = $tripsInfo['presented_to'];
            $main_name = $tripsInfo['main_name'];
            $location = $tripsInfo['location'];
            $tel = $tripsInfo['tel'];
        }
        ...
        echo $output = "<div class='modal-header'>
                ...
                <form action='#'>
                                ...
                                <div class='row'>
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly type='text' class='form-control wordsonly-format' value='$presented_to' name='ePresTo' id='ePresTo' placeholder='THE MANAGING DIRECTOR'>
                                        </div>
                                    </div>
                                    ...
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly type='text' class='form-control' name='eMaincomName' id='eMaincomName' value='$main_name' placeholder='AKWAABA LINK INVESTMENTS LTD.'>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class='row'>
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly type='text' class='form-control' name='eLocation' id='eLocation' value='$location' placeholder='SPINTEX LIGHT INDUSTRIAL AREA'>
                                        </div>
                                    </div>
                                    ...
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly readonly type='text' class='form-control' name='eComTel' id='eComTel' value='$tel' placeholder='0303 305730/0501530390'>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class='row'>
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly readonly type='text' class='form-control wordsonly-format' value='$transporter' name='tPresentedTo' id='tPresentedTo' placeholder='THE MANAGING DIRECTOR'>
                                        </div>
                                    </div>
                                    ...
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            <label>Company Main Name:</label>
                                            <input readonly type='text' class='form-control' name='tMaincomName' id='tMaincomName' value='$t_main_name' placeholder='KGL CONSTRUCTION LIMITED'>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class='row'>
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            ...
                                            <input readonly type='text' class='form-control' name='tLocation' id='tLocation' value='$t_address' placeholder='P.O.BOX CS 7967 ,   TEMA'>
                                        </div>
                                    </div>
                                    ...
                                    <div class='col-lg-6'>
                                        <div class='form-group'>
                                            <label>Company Tel.:</label>
                                            <input readonly type='text' class='form-control' name='tComTel' id='tComTel' value='$t_tel' placeholder='0303 305730/0501 530390'>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class='modal-footer'>
                        <button type='button' class='btn btn-link' data-dismiss='modal'>Close</button>
                        <button type='button' id='print_id_now_arc' class='btn bg-primary'><span class='ladda-label labll'>Print</span><i class='icon-printer ml-2 updateComIcon'></i></button>
                    </div>
                </form>";    
...
if(isset($_POST['prc'])){
    ...
    if($prc == "prc"){
        ...
        $lts_go = queryn($vSky, "SELECT DISTINCT `discharging_pt` FROM `arch_trips` WHERE `discharging_pt`!='$com_sub'");
