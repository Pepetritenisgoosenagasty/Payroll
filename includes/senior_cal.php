...
//      echo  " </tr>";
	...
	while( $excel->getActiveSheet()->getCell('A'.$i)->getValue() != ""){
		...
		$net_salary_before_other_deductions = $excel->getActiveSheet()->getCell('M'.$i)->getValue();
		$debt_bal_b_d =	$excel->getActiveSheet()->getCell('N'.$i)->getValue();
		$current_loan_surcharges = $excel->getActiveSheet()->getCell('O'.$i)->getValue();
		$total_debt = $excel->getActiveSheet()->getCell('P'.$i)->getValue();
		$noName = $excel->getActiveSheet()->getCell('Q'.$i)->getValue();
		$deductions = $excel->getActiveSheet()->getCell('R'.$i)->getValue();
		$balance_Outstanding = $excel->getActiveSheet()->getCell('S'.$i)->getValue();
		$total_Deductions =	$excel->getActiveSheet()->getCell('T'.$i)->getValue();
		$net_salary_payable = $excel->getActiveSheet()->getCell('U'.$i)->getValue();
		$bicycle_deductions = $excel->getActiveSheet()->getCell('V'.$i)->getValue();
		$netSalary = $excel->getActiveSheet()->getCell('W'.$i)->getValue();
...
//            $position;
       ...
       $smtm = mysqli_query($vSky, "INSERT INTO `employees`(`emp_name`, `emp_phone`, `emp_email`, `emp_bank_account_no`, `emp_bank_branch`, `emp_ssnit`, `emp_nationality`, `emp_additional_info`, `emp_allowances`, `emp_basic`, `emp_gross_salary`, `emp_taxable_salary`, `emp_paye`, `emp_total_statutory_deductions`, `emp_net_salary`, `emp_bicycle_deduction`, `empnet_salary_payable`, `emp_pos_id`, `emp_dept_id`, `emp_main_cat_id`, `ssf_id`, `input_date`, `last_update`, `is_pending`) VALUES ('".$name."', 'NULL', 'NULL', '".$bankAcc."', '".$bankBranch."', '".$ssnit."', 'NULL', 'NULL', '0', '".$basic."', '0', '".$taxable_salary."', '".$paye."', '0', '".$netSalary."', '".$bicycle_deductions."', '".$net_salary_payable."', '0', '0', '0', '0', '".$inputDate."', '".$inputDate."', '0')");
           ...
           // $statutory_deductions = round(sprintf("%0.2f", ( (basic*(5.5/100)) + $paye ) ) ,2);
            ...
            $deductions;
            $balance_Outstanding = round(sprintf("%0.2f",( $debt_bal_b_d+$current_loan_surcharges ) - $deductions ),2);
            $total_Deductions = round(sprintf("%0.2f",( ($basic*(5.5/100)) + $deductions) ),2);
            $net_salary_payable = round(sprintf("%0.2f",( $basic - ( ($basic*(5.5/100)) + $deductions) ) ),2);
            $bicycle_deductions;
            $netSalary = round(sprintf("%0.2f",(( $basic - ( ($basic*(5.5/100)) + $deductions) ) - $bicycle_deductions)),2);
		...
		$i++;
                ...
                if($smtm){
                    ...
                    echo 'Error no tax'.mysqli_error($vSky)."<br />";
                }
